node('docker') {
    ...
    stage('Docker deploy image') {
         ...
         docker build -f docker/azuredevopskats-web-build.dockerfile/Dockerfile --build-arg BUILD_NUMBER=${BUILD_NUMBER} --build-arg VCS_REF=`git rev-parse --short HEAD` -t stuartshay/azuredevopskats:2.1.9-build .'''
